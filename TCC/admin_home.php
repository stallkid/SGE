...
<!--FIM ANEXAR CURRICULO-->
    ...
    mysql_select_db('sge');
    $sql=("SELECT cod, empresa, curso, detalhes, data FROM vaga");
    ...
    $sqldel = "DELETE interesse_vaga,vaga  FROM interesse_vaga JOIN vaga ON interesse_vaga.cod_vaga = vaga.cod WHERE interesse_vaga.cod_vaga ='$value' AND vaga.cod = '$value'";
    $rsi = mysql_query($sqldel,$conexao);
      header("Refresh:0");
    }
    echo '<script language="javascript">
       alert("Vagas excluidas com sucesso!");
       window.location.href = "http://localhost:8080/TCC/admin_home.php";
    </script>';
    mysql_close($conexao);
    }
